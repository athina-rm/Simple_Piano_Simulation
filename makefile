BIN = proj# namn på output-binären
...
LIBS = -lprintf_flt -lm #for floating numbers and double
...
%.elf: $(OBJS) # Länka : ,-u,vfprintf, for floating numbers and double
	$(CC) -Wl,-u,vfprintf,-Map=$(@:.elf=.map) $(LDFLAGS) -o $@ $^ $(LIBS)
	$(AVRSIZE) $@
